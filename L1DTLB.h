...
class L1DTLB
{
    public:
        ...
        void Statistics(){
            ...
            if(m_b_counter != 0)
            cout << "[L1_" << m_id << "][Statistics]" << endl 
                 << "[Buffer][Total] " << dec << m_b_counter << endl
                 << "[Buffer][Hit] " << dec << m_b_hit << endl
                 << "[Buffer][Hit Rate] " << double(m_b_hit) / double(m_b_counter) * 100 << "%" << endl
                 << endl;
    ...
    private:
        ...
        unsigned int                        m_b_counter;
        unsigned int                        m_b_hit;
        ...
        array<DKEntry, 16>                  m_l1dk;
        array<Buffer, 16>                   m_l1db;
